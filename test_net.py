...
def parse_args():
  ...
  parser.add_argument('--img_path', dest='img_path',
                      help='sal val dataset',
                      default='../salicon/images/val', type=str)
...
if __name__ == '__main__':
  ...
  for i in lst:
    ...
    with torch.no_grad():
      pred = fasterRCNN.test(processed)
    pred = pred.squeeze().detach().cpu().numpy()
    pred = post_process(pred)
    pred_path = i.split('.')[0] + ".png"
    pred_path = os.path.join(outputdir, pred_path)
